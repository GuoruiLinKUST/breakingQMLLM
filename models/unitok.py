...
class UniTok(nn.Module):
    ...
    def forward(self, img, vae_bs, text=None, ret_usages=False):
        ...
        return output_dict
    def encoder_image_to_hidden(self, image):
        """
        Encode image to hidden representations via quantization indices.
        
        This function:
        1. Encodes image to tokens
        2. Projects to quantization space
        3. Finds the nearest codebook indices (quantization)
        4. Converts indices back to continuous embeddings
        5. Returns these embeddings WITHOUT post-quantization projection
        
        Args:
            image: torch.Tensor of shape (batch_size, channels, height, width)
            
        Returns:
            torch.Tensor of shape (batch_size, num_query, vocab_width)
            The quantized hidden states before post_quant_proj
        """
        # Encode image through the encoder
        ...
        #print('this is the shape for img_tokens 2', img_tokens.shape)

        return img_tokens

    def encoder_image_to_hidden_bpda(self, image):
        """
        Encode image to hidden representations via quantization indices.
        ...
        with torch.no_grad():
            img_indices = self.quantizer.f_to_idx(img_tokens)
            ff = (self.quantizer.idx_to_f(img_indices) - img_tokens).detach()

        # 将离散索引转换回特征空间
        img_tokens = img_tokens + ff
